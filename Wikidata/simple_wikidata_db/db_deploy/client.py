...
class MultiServerWikidataQueryClient:
    ...
    def query_all(self, method, *args):
        ...
        for res in results:
            if isinstance(res, str) and res == "Not Found!":
                continue
            elif isinstance(res, dict):  # <-- ADD THIS
                # Support merging dict results: update/merge
                if not is_dict_return:
                    # For functions like find_property_ids_for_relations, merge keys/values into a single dict
                    if not isinstance(real_results, dict):
                        real_results = {}
                    real_results.update(res)
                else:
